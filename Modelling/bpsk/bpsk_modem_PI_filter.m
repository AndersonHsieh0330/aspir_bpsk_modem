...
for i = 1:N
    if i>1
       ...
       phase_error(i-1) = q_arm_filtered(i-1) * i_arm_filtered(i-1);
       
       % PI control
       control_signal = Kp * phase_error(i-1) + Ki * integral_error(i-1);
       integral_error(i) = integral_error(i-1) + phase_error(i-1);
