...
module bpsk_demodulator_top (
    input  wire                              clk,
    input  wire                              rst,
    input  wire signed [`FIXDT_32_WIDTH-1:0] data_in, // from ADC, 2s complement
    output wire                              data_out // 1 or 0
);

wire signed   [`FIXDT_32_WIDTH-1:0] i_mixer_out_fifo [0:`LPF_TAPS-1];
wire signed   [`FIXDT_32_WIDTH-1:0] q_mixer_out_fifo [0:`LPF_TAPS-1];
wire signed   [`FIXDT_32_WIDTH-1:0] i_mixer_out, q_mixer_out;
wire signed   [`FIXDT_32_WIDTH-1:0] nco_carrier [0:1]; // [0] = cosine zero shift carrier(i), [1] = quadrature sine carrier(q)
wire signed   [`FIXDT_32_WIDTH-1:0] fb_mixer_out;
wire signed   [`FIXDT_32_WIDTH-1:0] i_lpf_out, q_lpf_out;
wire unsigned [$clog2(`CARRIER_SAMPLES_PER_PERIOD)-1:0] nco_i_cosine_lu_angle_steps, nco_q_cosine_lu_angle_steps;
wire signed   [`FIXDT_32_WIDTH-1:0] phase_adjust;

assign data_out = i_lpf_out[`FIXDT_32_WIDTH-1];
